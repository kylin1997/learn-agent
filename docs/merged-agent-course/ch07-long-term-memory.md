...
### 7.7.3 语义记录与关系图：双路检索，不要双写真相

向量检索擅长找“表达不同但含义相近”的记录；图关系擅长回答“谁与谁相关、哪个决定替代了哪个决定、某次故障依赖哪些条件”。两者可以组成双路记忆：
...
双路检索不等于每次对话都向向量库和图数据库各写一份真相。完整 claim、证据、scope、ACL 和版本仍只保存在权威记录中；Embedding 和关系边都是可重建索引。图边还要携带来源记录、有效期和关系类型，不能仅凭模型从闲聊中推断“某人与某人存在某种关系”就写入长期图谱。

更新索引时，可以由权威记录的提交事件驱动向量与图索引异步更新，并记录索引版本。某一路更新失败时，权威记录保持有效，系统重试派生更新；不能让两个后端分别接受模型写入，再尝试事后猜测哪一份更正确。只有查询确实涉及实体关系、谱系或多跳依赖时才扩展图候选，简单偏好查询不必固定支付双路成本。
...
一条记忆的原始措辞未必接近未来查询。例如记录写的是“高流量时连接池耗尽”，用户以后可能问“为什么只有并发上升时流水线才偶发超时”。写入或低频整理时，系统可以为已验证记忆生成少量可能的未来问法、别名和检索关键词，作为派生检索字段：

```yaml
memory_id: mem_pool_exhaustion
claim: 高流量时连接池耗尽会表现为间歇性超时
retrieval_aliases:
  - 为什么并发上升时才偶发超时
  - 已增加超时仍失败的可能原因
derived_from: mem_pool_exhaustion@v3
generator_version: query-expander-v2
```

这些问法只帮助“找到原记录”，不构成新事实、证据或用户意图。模型生成的问题可能加入原文没有的前提，也可能把罕见条件泛化成通用规律，因此它们应低权重参与候选召回，并能随源记录更新而重建。评估时同时观察 `Recall@k` 与 `Precision@k`：召回提高却带来大量误命中，说明增强问法扩得太宽。
...
### 7.9.1 分层压缩要保留回到原记录的路

当情景记录持续增长时，可以把记忆维护成三个层级：

```text
L0 原始记录：完整 claim、证据、时间、scope 和版本
L1 聚类摘要：同主题、同作用域、相近时间范围内的情景集合
L2 抽象经验：经过多次验证的语义结论或程序性策略
```

聚类不能只看向量距离，还要先按租户、scope、记忆类型、时间和实体边界分组，避免把不同用户或不同版本的相似记录压到一起。每条聚类摘要必须保存成员 ID、覆盖时间、保留的例外和冲突；生成后先用抽样回放检查事实覆盖，再降低原记录的默认召回权重。L1 或 L2 出错时，系统仍能回到 L0 重建，而不是让摘要取代证据。

分层压缩最常见的风险是“语义坍缩”：多数重复记录把少数关键例外淹没，时间顺序被抹平，模型生成的概括又被误写成已验证事实。高风险约束、用户明确纠正、失败边界和仍有冲突的记录应标记为不可自动合并。压缩产物本身也要经过写入门控、版本管理和可撤销发布。
...
还要区分**选择性遗忘**与用户要求的删除。选择性遗忘是检索治理：降低过期、低置信、已被替代或高度重复记录的召回权重，必要时将其归档；它不改变用户的数据权利，也不能代替硬删除流程。

遗忘策略可以综合有效期、证据强度、替代关系、重复度和任务影响。访问频率只能作为弱信号：一条错误记忆可能因为频繁召回而越来越“热门”，罕见但高影响的安全约束则可能长期不被访问。系统应保护用户明确要求、合规约束、关键失败边界和仍在争议中的少数记录，避免聚类或热度把它们自动清除。

执行顺序通常是：先生成候选与理由，模拟对召回指标的影响，再降低权重或归档，经过保留期后才考虑物理清理。模型可以提出遗忘候选，不能直接决定删除；用户纠正和删除请求始终走下面的权威记录流程。
...
| 语义库与图谱各自成为真相源 | 双写部分失败或语义漂移 | 单一权威记录，派生索引按提交事件重建 |
| 查询增强生成了不存在的前提 | 把派生问法当成事实 | 只作低权重检索别名，保留源记录与生成版本 |
| 聚类摘要抹掉少数例外 | 只优化压缩率 | 保留成员、冲突和不可合并标记，抽样回放后发布 |
| 高频错误记忆永不遗忘 | 把访问次数当真实性 | 结合证据、替代关系和任务影响，限制反馈偏差 |
...
11. 语义索引和关系图可以双路召回，但都应是同一权威记录的可重建派生物。
12. 查询增强、分层压缩和选择性遗忘都可能把模型推断误写成历史事实，必须保留来源、版本、例外和撤销路径。
...
13. 语义召回与图关系扩展分别适合解决什么问题，为什么不应各自维护一份权威记录？
14. 面向未来查询生成的别名为什么不能成为 claim 或 evidence？
15. 分层压缩怎样避免多数模式覆盖少数关键例外？
