# 第 11 章：Loop Engineering：从单次循环到可持续执行系统
...
## 11.1 控制权上移：从 Prompting 到 Loop Engineering
...
## 11.3 Loop 的本质不是 Repeat，而是 Feedback
...
>
> 这些命令、停止请求被评估器拦截的实现方式，以及 `/schedule`、Dynamic workflows 在文章发布时的研究预览状态，都是 Claude Code 的产品细节，不是 Loop Engineering 的通用原理。迁移到其他系统时，真正需要保留的是触发、状态、验证、停止、预算和治理关系。产品提供了更强的自动化入口，也不能绕过 [11.11](#1111-人不能退出责任闭环) 中的人类责任，或 [11.12](#1112-自治越强治理成本越高) 中的权限、安全 gate 与可恢复性要求。
...
第 11 章 Loop Engineering
  组织目标、反馈、状态、停止和人的治理

第 12 章 Multi-Agent
  提供分工、Maker-Checker、并行和隔离

第 13 章 Engineering / Observability
  提供测试、Trace、Metrics、Eval 和产品化
```

第 10 章回答“什么时候运行、消息如何进出”；第 11 章回答“为什么继续、如何证明完成、人在何处负责”；第 12 章回答“由谁分工执行和检查”；第 13 章回答“如何长期测量、测试和演进整套系统”。
