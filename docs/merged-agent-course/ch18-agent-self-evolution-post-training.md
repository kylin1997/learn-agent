# 第 18 章：Agent 自进化与后训练

> 本章目标：建立一条从运行数据到受控改进的闭环，并严格区分会话内适应、持久非参数资产更新与模型参数更新。读完后，你应该能把失败归因到 Context、Prompt、Skill、Memory、Tool、Workflow 或 Policy，能为三层变化设计不同的审批、灰度和回滚，能解释 SFT、偏好优化、RL 与 Agentic RL 的适用边界，也能识别数据污染、奖励投机和自修改带来的安全风险。
...
第 17 章负责建立可信的测试和评测信号，本章使用这些信号做改进决策。没有评测、审批和回滚的自进化，不是闭环，只是自动改写。
...
## 18.4 Prompt、Skill、Memory、Tool 与 Policy 怎样进化
...
### 18.6.5 失败轨迹不能直接作为逐 token SFT 目标
...
### 18.11.3 Shadow、Canary deployment 与自动回滚
...
| Skill / Plugin | 版本候选、供应链与发布闭环 | 第 11 章定义能力结构和加载 |
| 多 Agent | 从团队轨迹学习，治理共享经验 | 第 16 章定义委派、隔离和验证 |
| 评测 | 用回归门判断候选是否更好 | 第 17 章定义数据集、grader 和统计 |
| 生产发布 | 输出版本 manifest、灰度和回滚要求 | 第 19 章负责完整观测、配置和产品迭代 |
...
4. 失败轨迹不能原样作为 SFT 正标签；应截断、mask 错误动作、构造偏好对或只监督恢复段。
5. 奖励是可被策略利用的接口，安全约束不能只写成可抵消的小负分。
6. 候选生成事故集、目标验证集、能力回归集和 sealed holdout 必须按任务家族隔离。
...
5. 失败轨迹用于 SFT 时，何时应截断、loss mask、构造偏好对或只监督恢复段？
6. 稀疏奖励和密集奖励各自会产生什么问题？
7. 为什么 Judge、隐藏测试和权限 Policy 必须在自进化边界之外？
8. 训练数据如何避免与 holdout、contamination canary / hidden probe 和隐藏 grader 污染？
9. 为什么回滚模型版本不能自动撤销它已经造成的全部影响？
