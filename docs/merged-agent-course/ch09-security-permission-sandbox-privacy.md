...
8. 为高影响决策定义公平目标、忠实解释、申诉、更正和补救闭环，而不把单一指标或模型理由当作治理证明。
...
### 9.11.5 高影响决策治理：公平不是一个阈值

当 Agent 参与招聘、授信、医疗分诊、教育机会或公共资源分配时，安全不能只理解为“没有泄密、没有越权”。一次完全按权限执行的决定，仍可能系统性地让某些人群承担更高误拒率，或让错误通过反馈数据被下一轮模型继续放大。

治理对象应从单次工具调用扩展为完整决策闭环：
...
Decision Context
  ...
  -> Explanation
  -> Appeal / Correction / Remedy
```

来源中的伦理 Agent 把义务、允许和禁止编码为确定性检查器，并把检查点放在工具执行之前。这个结构值得保留，但它只是**可执行 Policy**，不是把全部价值冲突变成了数学真理。哪些利益应优先、采用什么公平目标、允许多大代价和谁有最终裁决权，仍是需要受影响者、领域专家和责任主体参与的规范性选择。

常见公平指标回答的是不同问题：

| 指标 | 主要问题 | 不能单独证明什么 |
| --- | --- | --- |
| 结果比例 | 不同群体获得正向结果的比例是否接近 | 没有说明资格、标签质量和个体误差 |
| 机会或召回 | 在符合目标条件的人中，不同群体被正确识别的比例是否接近 | 依赖“真实条件”标签可信，可能牺牲其他误差 |
| 精度或校准 | 获得同一分数或正向决定后，实际结果是否同样可靠 | 基础发生率不同或标签有偏时，群体结果仍可能不同 |

现实中这些目标经常无法同时最优。工程团队不能先挑一个让仪表盘变绿的指标，再把选择伪装成中立事实。每个高影响用例至少要记录适用人群、目标指标、误差成本、排除规则、标签来源、阈值版本、审批者和复核周期。

删除受保护字段也不等于消除偏差。地区、学校、语言风格、设备和历史行为可能成为代理变量；完全删除群体信息还会让系统无法测量差异。更稳健的做法是把**决策用途**与**受控审计用途**分开授权，在合法、必要且最小化的前提下监测分群结果，并限制原始身份数据进入模型和普通日志。

### 9.11.6 可解释、可申诉与可补救

“Agent 给出了一段理由”不等于决定可解释。至少要区分四种不同资产：

- **执行轨迹**：实际调用了什么模型、规则、工具和数据版本。
- **决策证据**：哪些可验证输入与约束支持或反对结果。
- **用户解释**：用适合受众的语言说明主要因素、限制和下一步。
- **模型内部推理**：可能不可见、不稳定，也不构成可靠审计日志。

解释应从真实执行轨迹、规则命中和证据引用生成。让另一个模型在事后编一段流畅理由，可能只是合理化，并不能证明它忠实于原决定。模型特征归因、规则解释和反事实回答的对象也不同：归因说明哪些输入影响结果，规则解释说明哪个门槛生效，反事实说明什么可行变化可能改变结果。反事实必须排除不可控、受保护或不应要求用户改变的属性，否则“可操作建议”会变成新的伤害。

解释层还应绑定置信与失败语义。置信度要通过历史结果校准，不能直接采用模型自报数字；数据陈旧、关键证据缺失、规则冲突或解释生成失败时，高影响决定应暂停、降级或交给人，而不是交付一个没有依据但语气肯定的结论。

真正的申诉机制需要让受影响者能够查看决定依据、指出数据错误、提交新证据并获得独立复核。若复核推翻原决定，系统还要能更正状态、阻止错误继续写入训练或记忆资产，并在适用时执行撤销、重新处理或补救。只有解释却没有改变决定的通道，仍然只是单向告知。
...
10. **决策治理测试**：按适用人群和风险分片比较误拒、误放、升级和撤销，验证代理变量、阈值漂移、解释忠实度、申诉与更正闭环。
...
8. 高影响决策还必须可比较群体结果、忠实解释依据、接受独立申诉并修正后果；单一公平指标或事后理由都不够。
...
9. 为什么删除受保护字段既不能证明公平，也可能让团队失去监测差异的能力？
10. 执行轨迹、决策证据、用户解释和模型内部推理为什么不能混成一份“理由”？
...
11. 当两个公平目标无法同时满足时，谁有权选择取舍，怎样让受影响者参与并使选择可复核？
12. 反事实解释怎样避免暗示用户应改变受保护属性，或承诺系统实际上不会接受的补救路径？
...
- [30 Agents：第 4 章，部署、零信任与公平性审计](../../source/30-Agents-Every-AI-Engineer-Must-Build/chapter04/ch04_agent_deployment.ipynb)
- [30 Agents：第 12 章，伦理推理与公平性治理](../../source/30-Agents-Every-AI-Engineer-Must-Build/chapter12/ch12_01_ethical_reasoning_agent.ipynb)
- [30 Agents：第 12 章，可解释、反事实与置信沟通](../../source/30-Agents-Every-AI-Engineer-Must-Build/chapter12/ch12_02_explainable_agent.ipynb)
