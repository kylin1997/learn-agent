# 第 12 章：MCP、A2A、ANP 与 Agent 互操作
...
这里的 Skill 是 Agent Card 中对远端能力的描述，不等同于第 11 章的本地 `SKILL.md` 文件。相同名词在不同协议层可能有不同数据模型。

### 12.5.3 A2A 不要求固定网络拓扑
