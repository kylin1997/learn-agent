...
- 权限、沙箱和隐私治理属于第 10 章；框架的回调或中间件不能自动构成安全边界。
...
## 9.3 四层抽象：不要把 SDK、框架、运行时和平台混为一谈
...
## 9.5 选择编排形态：Loop、Chain、DAG、Graph 与 Durable Workflow
...
### 9.6.2 Model Adapter：统一能力，不假装完全相同
...
### 9.6.3 Tool Contract：Schema、策略与执行分离
...
## 9.14 可视化平台：区分 Agent 应用与通用工作流自动化
...
- 第 7 章的长期记忆可以通过 Store、Tool 或 Middleware 接入，但必须保持跨会话治理边界。
- 第 8 章的 Retriever 可以成为 Runnable、Tool 或图节点；检索证据契约不依赖具体框架。
- 第 10 章提供权限和沙箱；本章的 Tool Registry 只负责发现和调用入口。
