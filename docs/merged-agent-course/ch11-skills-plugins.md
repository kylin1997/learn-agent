...
本章只讨论 Skills 与插件。Tool 作为它们依赖的原子执行能力会被用于说明边界，但不重复工具运行时；MCP 及其工具发现、连接和授权机制统一放在第 12 章，不在本章展开。插件可以声明外部协议组件，但声明和治理属于插件，协议行为仍属于互操作层。

## 11.2 为什么需要 Skill 这一中间层
