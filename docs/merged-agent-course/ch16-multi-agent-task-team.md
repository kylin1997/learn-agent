...
> 本章目标：学会判断何时值得引入多 Agent，并把多 Agent 从“多个模型同时说话”落实为任务图、委派契约、隔离边界、通信协议、工作区和独立验证。读完后，你应该能用“是否产生独立新信息”检验协作价值，能分别选择上下文共享方式与协作拓扑，能设计 Lead、Worker、Verifier 的职责，能实现带依赖和原子认领的任务 DAG，也能识别团队规模扩大后出现的协调失败。
...
### 16.2.2 更严格的判据：协作有没有带来独立新信息

角色数量不是有效性的证据。对于同一个输入，如果多个 Agent 使用相同模型、相同上下文、相同工具和相同验证方式，它们往往只会产生相关性很高的重复采样。讨论更长、票数更多，都不能保证系统知道了原先不知道的事实。

一个 Worker 值得加入团队，至少应带来一类主 Agent 在当前生成步骤中拿不到的信息：

| 新信息来源 | 例子 | 为什么有价值 |
| --- | --- | --- |
| 环境反馈 | 编译错误、测试结果、页面截图、数据库最终状态 | 它来自真实执行，不是模型再次猜测 |
| 独立数据源 | 不同文档库、不同传感器、受隔离权限保护的数据 | 它扩大证据覆盖面 |
| 独立方法 | 符号验证器、静态分析器、约束求解器 | 它采用不同失效机制检查同一结论 |
| 独立观察 | 不共享实现者结论的盲审、人工抽检 | 它降低确认偏差和同源偏差 |

因此，“Writer 写完后让 Reviewer 再读一遍”只有在 Reviewer 能运行测试、查看渲染结果、核对外部事实，或依据独立验收标准盲审时才有稳定收益。若 Reviewer 只能对同一段文本发表意见，先把预算用于一次更强的单 Agent 执行、确定性检查或外部工具反馈。

可以把拆分决策改写成两个连续问题：

1. 新执行单元会获得什么独立信息，或隔离什么必须隔离的上下文、权限与写集合？
2. 这份增量能否通过工件、环境状态或独立判定被 Lead 验证？

两个问题都答不清时，多 Agent 只是把一次不确定生成变成多次相关生成。
...
设计多 Agent 时要分开处理两个维度：

- **上下文共享程度**决定每个 Agent 看见多少历史。它可以从完整继承、共享稳定前缀加结构化交接，变化到完全隔离。
- **协作拓扑**决定控制权和工件怎样流动，例如父子委派、中心协调、流水线或对等协作。

这两个维度相互影响，但不是同一个选择。完整共享上下文的团队也可以采用链式 Handoff；不共享上下文的 Worker 既可以受 Coordinator 调度，也可以从任务池自主认领。工程评审时应分别记录两个决定，否则“我们采用 Supervisor 架构”并没有回答 Worker 是否复制完整 transcript，也没有回答敏感数据怎样隔离。

| 上下文策略 | 适合情况 | 主要代价 |
| --- | --- | --- |
| 完整共享 | 角色少、串行交接、细节零损耗是硬要求 | 上下文膨胀、缓存与隐私成本增加 |
| 稳定前缀 + 结构化交接 | 多数工程团队；共享目标与规则，只传必要工件 | 需要维护 handoff schema 和版本 |
| 完全隔离 | 独立验证、敏感分区、大规模并行 | 信息同步、冲突处理和调试成本更高 |

选完上下文策略，再选择下面的拓扑。不要用“角色切换”掩盖上下文复制，也不要用“独立 Agent”暗示它们的证据已经独立。
...
### 16.6.3 虚拟文件系统要按可见性和生命周期分区

多 Agent 通过统一的文件接口协作，不代表所有文件都应该放进同一个共享目录。运行时可以把不同存储挂载成一棵虚拟目录树，但必须保留四类区域的语义：

| 区域 | 可见性与用途 | 生命周期 | 默认权限 |
| --- | --- | --- | --- |
| Agent Scratchpad | 单个 Worker 的草稿、临时日志和试错产物 | 随 Worker 回收 | 私有读写 |
| Shared Workspace | 团队交换且用户可见的正式工件 | 随任务持久化 | 受版本和并发控制的读写 |
| External Mount | 用户授权的云盘、知识库或业务系统 | 由外部系统决定 | 优先只读，写回单独审批 |
| Built-in Resources | Skill、模板、规则和参考手册 | 跨任务稳定 | 全局只读 |

目录分区解决三类问题。Worker 的试错不会污染正式交付；外部数据的凭据和一致性规则不会被普通文件接口掩盖；系统内置规则也不会被任务中的 Agent 改写。Handoff 传递工件路径、版本和摘要即可，大文件不必复制进每个上下文。

共享区域仍需乐观锁、revision 或 Worktree。文件路径只是一种统一接口，不能替代权限检查、来源标签和并发控制。
...
| 错误级联 | 上游猜测被下游当事实，越传越确定 | 消息缺少证据、置信度和来源，接收者只做语言润色 | 传递可核验工件；关键结论独立复验；限制传播深度 |
...
错误级联与普通单点失败不同。一个 Worker 的错误一旦进入共享状态，后续 Agent 可能把它当作已确认前提，再补充更流畅的解释，最后让 Lead 看到一份内部一致但整体错误的报告。消息信封应区分 `observation`、`inference` 与 `decision`，并携带来源、工件哈希和验证状态。关键前提不能只靠更多 Agent 复述，应回到原始数据或独立环境重跑。

### 16.13.1 Agent society 目前只提供研究问题

开放式 Agent 社会研究了长期记忆、信息传播、声誉、市场协作和群体涌现。这些实验能帮助我们提出问题，例如错误信息怎样扩散、局部目标怎样与全局目标冲突、自治成员怎样形成联盟。但研究模拟中的社交行为、奖励提升或群体规模不能直接证明生产系统可靠。

工程系统仍要使用本章的明确责任链、权限边界、任务终态和可复现实验。只有当 Agent society 的某个机制可以写成协议、指标和故障测试时，才把它纳入产品设计；其余内容保留为研究边界。
...
| 团队内部协议 | 定义消息语义、状态和幂等 | 第 11 章负责跨产品互操作标准 |
...
| 经验沉淀 | 只提交候选经验，不直接改全局能力 | 第 19 章负责自进化闭环 |
...
各来源从不同层面指向同一个工程结论。`learn-claude-code` 和 Harness Engineering 给出 TaskList、Claim、Mailbox、Team 与 Worktree 的运行时结构；Hermes 强调委派预算、中断、深度和认知隔离；Alice 说明角色、并发、二次审查和收敛信号；Claude Code 分析补充进程内上下文与权限实现；`easy-langent` 展示 Supervisor、Sequence、Peer 和并行状态合并；`claw0` 提供 lane 与 generation；Hello-Agents 和 `hello-claw` 展示框架级与产品级团队案例；《AI Agents in Action（第二版）》补充协调策略、Agent/Flow 选择、Handoff 追踪和评估器共同失效治理；《深入理解 AI Agent》第 10 章补充了上下文共享与协作拓扑两个独立维度、新信息判据、虚拟文件系统分区和错误级联。

可以把本章压缩为十一条结论：

1. 多 Agent 的成立条件是分工收益大于协调成本，协作价值还要说明它带来了什么独立新信息。
2. 上下文共享程度与协作拓扑是两个设计轴，必须分别选择和记录。
3. Task DAG 是调度内核；原子认领、单调 lease epoch 和 fencing token 共同阻止旧 Worker 晚到提交。
4. 委派要交付目标、范围、权限、预算、输出协议和验收条件。
5. 隔离包括身份、上下文、状态、工具、权限、文件系统和生命周期。
6. 虚拟文件系统应区分私有草稿、任务共享、外部挂载和只读内置资源。
7. Worker 权限要分别收窄可委派动作和每个动作的资源 scope，并对 `(action, resource)` 授权。
8. Worktree 隔离文件修改，但仍需隔离端口、数据库、账号、缓存和密钥。
9. Lead 负责全局综合，Verifier 负责独立判定，两者都不能被结果拼接替代。
10. 团队完成必须由可验证工件和明确终态证明；相关 Agent 的一致复述不是验证。
11. Agent society 当前提供研究问题，生产采用其中任何机制都要先把它改写为协议、指标和故障测试。

## 16.18 本章自检

1. 什么条件下增加一个 Subagent 的收益会超过它的上下文和通信成本？
2. 为什么“多个 Agent 给出相同答案”不能证明协作产生了独立新信息？
3. 上下文共享程度与协作拓扑为什么不能合并成一个架构标签？
4. Todo 与 Task DAG 在作用域、依赖和并发语义上有什么根本差别？
5. 为什么任务认领既需要原子化，也需要 lease epoch 和 fencing token？哪些提交点必须校验？
6. 一份完整委派包必须包含哪些字段？
7. 为什么不能把工具动作、文件路径和网络 scope 放进同一个集合求交？
8. Scratchpad、Shared Workspace、External Mount 和 Built-in Resources 的权限与生命周期有何区别？
9. Worktree 没有隔离哪些常见资源？
10. 错误为什么会在多个 Agent 之间被放大，而不是被人数自动纠正？
11. 团队系统怎样证明自己已经收敛，而不是暂时没有新消息？
...
11. Agent society 中出现的涌现协作，要满足哪些可复现性和控制条件才适合迁移到生产系统？
...
- [深入理解 AI Agent：第 10 章，多 Agent 协作](../../source/ai-agent-book/book/chapter10.md)
- [深入理解 AI Agent：第 10 章配套实验](../../source/ai-agent-book/chapter10/README.md)
