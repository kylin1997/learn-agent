...
> 本章目标：建立从确定性单元测试到线上实验的完整质量体系。读完后，你应该能区分测试、评测、基准与观测，用 TDAD 在修改 Agent 前定义正确性，同时评价最终结果和执行轨迹，为 LLM-as-judge 设定边界，设计防污染的本地基准集与回归门，并用重复运行、置信区间和配对比较解释非确定性结果。
...
可观测性为评测提供证据，但“记录了很多 trace”不等于“知道系统好不好”。训练和自进化使用评测结果改进系统，第 18 章负责；本章只建立可信评价信号。
...
标注可以指出“这里有问题”，但不能自动提供正确答案。缺少可复现环境或可靠裁决的样本先保留为调查材料，不要强行制作金标。第 18 章只消费已经治理、版本化并有明确用途的回归或改进资产；第 19 章负责生产事件和标注入口。
...
Hermes 提供隔离、分层、超时和稳定性专项测试；Hello-Agents 把 BFCL、GAIA、LLM Judge、Win Rate 与人工验证放进同一评估框架；Harness Engineering 提供 Trace、Feature Flag 和 A/B 的生产链路；Alice、`claw0`、`learn-claude-code` 与 Claude Code 分析提供多 Agent、恢复、权限和反馈等可观测行为；`easy-langent` 与 `hello-claw` 的应用案例则提醒我们，任务成功必须落到环境状态和场景价值。

本章的核心结论是：

1. 确定性运行时用精确测试，概率性策略用任务分布和重复运行评价。
2. 结果、轨迹、系统不变量和产品结果是四类证据，不能互相替代。
3. LLM Judge 是需要校准的测量工具，不是事实来源或安全裁判。
4. TDAD 要求先定义正确性、建立重复基线、做最小可归因改动，再运行完整回归。
5. 公共基准提供能力坐标，本地基准覆盖真实产品风险。
6. 数据污染包括训练泄露、环境泄露、Memory 泄露和评测过拟合。
7. 回归门要保留安全硬门、能力非退化和资源预算，不能只看总分。
8. 比较版本要使用配对设计、重复运行、分片结果、效应量和不确定性。
9. 线上信号要经过实验、审计和脱敏，才能转化为新的评测案例。

## 17.18 本章自检

1. 测试、评测、基准和可观测性分别回答什么问题？
2. 哪些 Agent 组件应该完全不调用模型就能测试？
3. 为什么轨迹评测不能只比较“是否走了参考步骤”？
4. 端到端任务为什么应该验证环境状态，而不是只比较最终文本？
5. LLM Judge 有哪些系统性偏差，怎样做盲化和校准？
6. 随机拆分近重复 case 为什么会造成错误的泛化结论？
7. 为什么同一 case 的多次运行不能全部当作独立样本？
8. 安全门、能力门和成本门应该怎样共同决定发布？
9. 线上点击或点赞为什么不能直接作为训练奖励？
