...
> 本章目标：建立从确定性单元测试到线上实验的完整质量体系。读完后，你应该能区分测试、评测、基准与观测，能同时评价最终结果和执行轨迹，能为 LLM-as-judge 设定边界，能设计防污染的本地基准集与回归门，也能用重复运行、置信区间和配对比较解释非确定性结果。
...
4. 公共基准提供能力坐标，本地基准覆盖真实产品风险。
5. 数据污染包括训练泄露、环境泄露、Memory 泄露和评测过拟合。
6. 回归门要保留安全硬门、能力非退化和资源预算，不能只看总分。
7. 比较版本要使用配对设计、重复运行、分片结果、效应量和不确定性。
8. 线上信号要经过实验、审计和脱敏，才能转化为新的评测案例。
