...
本章讨论的是 Agent Harness 的治理面，不展开某个产品的配置命令，也不把加密、容器或审批中的任何一种机制描述成万能方案。MCP 等互操作协议的协议机制放在第 12 章；本章只讨论它们跨越信任边界时必须接受的统一治理。

## 10.2 为什么 Agent 改变了安全问题
...
## 10.5 从 Policy 到执行的权限管线

### 10.5.1 Policy 是机器可执行的治理契约
...
## 10.9 Prompt Injection：把内容攻击阻断在动作边界
