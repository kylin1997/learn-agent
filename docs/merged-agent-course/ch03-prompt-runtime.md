...
> 目录说明：本文件暂时沿用旧版第 3 章编号；在新版 20 章目录中对应第 4 章“Prompt Engineering 与 Prompt Runtime”。整章重构时将统一文件名和节号。
>
...
### Prompt 债务：为什么提示词会越修越长

Prompt 最常见的退化方式不是少写了一句，而是每次遇到失败都追加一句。一次工具误用，补一条工具规则；一次输出过长，补一条风格要求；一次任务提前结束，再补一段完成条件。局部补丁可能修复眼前案例，却会逐渐形成 **Prompt 债务**：重复要求、互相冲突的流程、过时的模型补偿、只对单个案例有效的示例，以及当前任务根本用不到的工具说明同时常驻。

Prompt 债务会带来四类成本：

- **注意力成本**：真正重要的目标和约束被大量低价值文字稀释。
- **决策成本**：模型要解释多个相似或冲突的规则，行为反而更不稳定。
- **运行成本**：输入 token、延迟和缓存失效增加。
- **维护成本**：团队不敢删除任何一句，因为不知道它当初修复了哪个失败。

因此，优化 Prompt 不只是“怎样写得更完整”，还包括 **有证据地删除**。可以先给每个 section 做一次审计：

| 检查项 | 应保留 | 应优先删减或移出常驻 Prompt |
| --- | --- | --- |
| 目标 | 可验证的最终结果 | 空泛身份包装、重复目标 |
| 约束 | 安全、权限、证据和业务不变量 | 已由权限系统强制执行的重复说明 |
| 流程 | 确有顺序依赖的关键节点 | 对所有可能路径的逐步穷举 |
| 示例 | 能覆盖难判边界的少量示例 | 只复现单个历史故障的大量示例 |
| 工具 | 当前任务可用且相关的工具 | 已禁用、无关或功能重叠的工具 |
| 输出 | 验收所需的字段和验证方式 | 与任务无关的固定文风和篇幅要求 |

删除不是凭感觉“精简文案”。每次删减都要由回归任务证明：成功率、约束遵守和结果质量没有下降，或者总体收益高于局部退化。

### Outcome-first：描述终点，不穷举所有路径

Agent Prompt 容易写成一份僵硬操作手册：先做 A，再做 B，然后做 C；如果 B 失败就做 D。问题是 Agent 面对的环境不完全可预测，最佳路径会随代码库、工具返回和中间证据变化。把所有路径写死，会限制模型利用新证据调整计划。

更稳健的写法是 **Outcome-first**，先定义五件事：
...
例如，不要只写“先搜索三个文件，再修改代码并运行测试”，而应写成：

```text
目标：修复用户报告的缺陷，并保持改动范围最小。
成功标准：相关复现用例由失败变为通过，邻近测试没有新增失败。
约束：不得修改无关模块，不得隐藏或删除失败测试。
权限：涉及远端写入、发布或不可逆操作时先请求确认。
停止：成功标准满足后结束；若关键输入缺失且调查无法补足，则报告证据和阻塞点。
```

这份 Prompt 规定了终点、护栏和验收方式，却允许 Agent 根据实际证据选择有效路径。流程仍然有价值，但只应固化真正的不变量和依赖关系。

### 不变量、决策规则与停止规则

不是所有规则都应该使用“必须”或“永远”。过多绝对指令会制造冲突，也会让正常例外变成违规。可把规则分成三类：

| 规则类型 | 适用问题 | 写法 |
| --- | --- | --- |
| 不变量 | 安全底线、权限边界、数据完整性、协议要求 | 明确使用“必须/禁止”，并由代码侧兜底 |
| 决策规则 | 是否搜索、是否并行、是否询问、选择哪种工具 | 给判断条件、优先级和例外 |
| 停止规则 | 何时交付、何时求助、何时放弃一条路径 | 给成功、阻塞、预算和风险阈值 |

停止规则尤其重要。没有停止条件的 Agent 可能反复搜索、持续润色、重复运行同一工具，或者在已经满足需求后继续“顺手改进”。一个完整的停止契约至少覆盖：

- **成功停止**：验收标准已经满足，验证证据已经收集。
- **阻塞停止**：缺少关键输入，且在限定调查范围内无法获得。
- **风险停止**：下一步会越过权限边界或产生不可接受的副作用。
- **预算停止**：达到轮次、时间、token 或成本上限，必须总结状态并移交。

Prompt 可以描述这些规则，Loop Runtime 则负责计数、超时、权限拦截和强制终止。两者结合，停止条件才不是一句容易被忽略的提醒。

### 验证契约：完成不等于模型说“完成”

成功标准说明什么结果算完成，验证契约进一步规定 **用什么证据判定完成**。如果 Prompt 只要求“检查结果”，模型可能把重新阅读自己的答案当成验证；如果契约要求独立证据，Agent 才知道必须运行测试、读取实际工件或核对权威来源。

验证契约可以按任务类型定义：

| 任务 | 可接受证据 | 不足以单独证明完成的信号 |
| --- | --- | --- |
| 代码修改 | 复现用例通过、相关测试通过、静态检查或构建结果 | 代码看起来正确、模型自述已修复 |
| 信息研究 | 直接来源、发布日期、交叉核对和引用位置 | 搜索摘要、没有出处的转述 |
| 数据处理 | 行数与约束检查、抽样核对、可重复计算 | 脚本正常退出 |
| 界面实现 | 目标视口截图、交互测试、错误日志检查 | 页面能够打开 |

Prompt 负责告诉模型需要收集哪些证据；工具和评测系统负责产生、保存并核验这些证据。高风险任务还要区分“已验证”“部分验证”和“无法验证”，禁止模型把缺少验证包装成确定结论。
...
工具集合本身也是 Prompt 的一部分。一次调用暴露的工具越多，模型越需要在相似能力之间消歧，工具 Schema 也会占用更多上下文。Prompt Runtime 应根据任务阶段和权限状态裁剪工具，只暴露当前相关、当前可用的最小集合；被权限系统禁用的工具不应一边暴露、一边期待模型“自觉不用”。

工具策略还要说明调用关系：互不依赖的只读查询可以并行，有数据依赖或副作用顺序的调用必须串行。工具失败时，不应无限重试，也不应把空结果直接解释成事实不存在。更可靠的规则是：检查错误类型，在有限预算内换查询、缩小范围或使用一个明确的回退来源；仍无结果时，把结论标记为“未找到证据”，而不是“已证明为假”。

对于检索类工具，可以把预算直接写进契约：允许多少次查询、最多展开多少候选、哪些来源优先、什么证据足以停止。这样，模型既不会命中第一条摘要就草率下结论，也不会为了追求“绝对完整”无限搜索。

某些模型或 API 支持程序化工具调用（Programmatic Tool Calling，PTC），让模型在推理过程中生成并执行受限代码，以组合工具、过滤结果或完成中间计算。这类能力可减少模型与工具之间的往返，但它属于 **Provider 运行时能力**，不能当成通用 Prompt 技巧。系统仍需限制可调用工具、代码环境、网络与文件权限、执行时间和输出大小，并保留调用轨迹供验证。
...
模型升级或 Agent 行为漂移时，同时改模型、重写 Prompt、调整工具并更换评分规则，会让结果无法归因。即使结果变好，也无法知道是哪项变化生效；结果变差时更无法定位原因。可靠的迁移顺序是：

1. 保存旧模型、旧 Prompt 在代表性任务集上的基线和 Trace。
2. 只替换模型，保持 Prompt、工具和评测标准不变，观察能力与失败模式变化。
3. 删除已过时的脚手架、重复规则、无关示例和无关工具，每次只改一类内容。
4. 对剩余的真实失败添加最小、可解释的修复，而不是恢复整段旧补丁。
5. 每次改动后重跑相同任务集，同时比较任务成功、约束遵守、工具轨迹、token、延迟和成本。

评测不应只看最终文本。Agent 可能碰巧得到正确答案，却走过危险或不可复现的路径。Trace 评测还要检查：是否检索了必要证据，是否选择了正确工具，是否在权限节点停下，是否重复调用，是否完成验证，以及停止原因是否与真实状态一致。

这也是“单变量改动”的价值：Prompt 的每一段都应能够回答“它解决哪个可复现失败，用什么案例证明仍然必要”。回答不了的内容，就是下一轮减法审计的候选。

### GPT-5.6 案例：把模型专属控制留在运行时

OpenAI 的 GPT-5.6 Prompt 指南给出过一个内部编码 Agent 的迁移案例：在删除过时脚手架、重复指令和无关工具后，评测分数提高约 10%–15%，总 token 减少约 41%–66%，成本减少约 33%–67%。这些数字只描述该 Harness 与任务分布下的实验结果，用来说明“删减后仍需评测”，不能外推成所有 Agent 的收益承诺。

这份指南还涉及 `text.verbosity`、`reasoning.effort`、工具搜索、持久化推理项和 assistant phase 等 API 能力。学习时应把它们放在正确层级：

- `text.verbosity` 控制回答展开程度，不替代输出 Schema 和验收标准。
- `reasoning.effort` 是需要通过任务评测选择的计算预算，不是越高越正确。
- 工具搜索和程序化工具调用优化的是工具暴露与调用效率，不替代权限和验证。
- 持久化推理项有助于长任务连续性，但目标、假设或优先级变化后，旧推理也可能把系统锚定在过时路径上。
- assistant phase 等回放字段属于 Provider 消息协议。系统应按协议原样保留，不能把它与持久化推理一起降权、摘要或失效。

因此，通用教材主线仍是目标、约束、工具契约、停止条件和评测；模型专属参数由模型运行时或 Provider Adapter 管理，并在目标模型和真实任务集上单独校准。Prompt Runtime 只消费适配后的能力配置，负责选择和组装本次调用需要的 Prompt section。
...
历史补丁、重复流程和无关工具会形成 Prompt 债务；删除同样需要回归证据。

**错误九：把模型专属参数当成通用提示技巧。**

verbosity、reasoning effort、工具搜索和推理持久化属于 Provider 能力，应由 Runtime 适配。
...
8. 为每个 section 记录它保护的失败案例，定期执行 Prompt 债务审计。
...
5. Prompt 优化既包括增加必要约束，也包括用评测删除过时脚手架、重复规则和无关工具。
...
11. Prompt 债务是怎样形成的？为什么不能凭感觉做减法？
12. 不变量、决策规则和停止规则分别应该怎样表达？
...
4. 模型升级后，哪些旧规则应该先保留用于建立基线，哪些内容最可能成为待删除的脚手架？
5. “未检索到证据”和“事实为假”有什么区别？Agent 的检索与停止契约应怎样避免混淆二者？
