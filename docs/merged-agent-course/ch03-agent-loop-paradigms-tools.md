...
### 3.5.3 Reflection 与 Reflexion：反馈循环是否进入记忆
...
### 3.5.4 Evaluator-Optimizer：预定义 Workflow
...
范式跨越不同层：ReAct 位于动态控制循环；Plan-and-Solve 原始形式是提示策略，计划物化后才进入状态控制；Reflection 是局部反馈修订循环；Reflexion 再增加反馈记忆；Evaluator-Optimizer 的拓扑由代码预先定义，属于 Workflow。

## 3.16 共同结论

1. Agent Loop 的本质是模型决策与环境反馈构成的闭环。
2. ReAct、Plan-and-Solve、Reflection/Reflexion 与 Evaluator-Optimizer 分属动态控制、提示/计划、反馈记忆和预定义 Workflow，不应被当作同层替代品。
...
5. Reflection 与 Reflexion 在反馈记忆上有什么区别？
6. Evaluator-Optimizer 为什么属于预定义 Workflow？
7. 工具契约应包含哪些函数签名无法表达的元数据？
8. `is_read_only` 与 `is_concurrency_safe` 为什么不能画等号？
9. Prompt Runtime 为什么只能缩小 Policy 的工具授权集？
10. 为什么“模型不再调用工具”不足以证明任务完成？
