...
```bash
mkdir -p $(dirname .omc/state/team/worker-1-source-9-slo-worker-2/workers/worker-2/.ready) && touch .omc/state/team/worker-1-source-9-slo-worker-2/workers/worker-2/.ready
```
...
1. **Claim** your task (run this command first):
   `omc team api claim-task --input "{\"team_name\":\"worker-1-source-9-slo-worker-2\",\"task_id\":\"<id>\",\"worker\":\"worker-2\"}" --json`
...
3. **Send ACK** to the leader:
   `omc team api send-message --input "{\"team_name\":\"worker-1-source-9-slo-worker-2\",\"from_worker\":\"worker-2\",\"to_worker\":\"leader-fixed\",\"body\":\"ACK: worker-2 initialized\"}" --json`
...
- **Worker**: worker-2
- **Agent Type**: codex
- **Environment**: OMC_TEAM_WORKER=worker-1-source-9-slo-worker-2/worker-2
...
- Claim task: `omc team api claim-task --input "{\"team_name\":\"worker-1-source-9-slo-worker-2\",\"task_id\":\"<id>\",\"worker\":\"worker-2\"}" --json`
...
- Release claim (rollback): `omc team api release-task-claim --input "{\"team_name\":\"worker-1-source-9-slo-worker-2\",\"task_id\":\"<id>\",\"claim_token\":\"<claim_token>\",\"worker\":\"worker-2\"}" --json`

## Communication Protocol
- **Inbox**: Read .omc/state/team/worker-1-source-9-slo-worker-2/workers/worker-2/inbox.md for new instructions
- **Status**: Write to .omc/state/team/worker-1-source-9-slo-worker-2/workers/worker-2/status.json:
  ```json
  {"state": "idle", "updated_at": "<ISO timestamp>"}
  ```
  States: "idle" | "working" | "blocked" | "done" | "failed"
- **Heartbeat**: Update .omc/state/team/worker-1-source-9-slo-worker-2/workers/worker-2/heartbeat.json every few minutes:
  ```json
...
- To leader: `omc team api send-message --input "{\"team_name\":\"worker-1-source-9-slo-worker-2\",\"from_worker\":\"worker-2\",\"to_worker\":\"leader-fixed\",\"body\":\"<message>\"}" --json`
- Check mailbox: `omc team api mailbox-list --input "{\"team_name\":\"worker-1-source-9-slo-worker-2\",\"worker\":\"worker-2\"}" --json`
- Mark delivered: `omc team api mailbox-mark-delivered --input "{\"team_name\":\"worker-1-source-9-slo-worker-2\",\"worker\":\"worker-2\",\"message_id\":\"<id>\"}" --json`
...
1. Write your decision to: .omc/state/team/worker-1-source-9-slo-worker-2/workers/worker-2/shutdown-ack.json
2. Format:
